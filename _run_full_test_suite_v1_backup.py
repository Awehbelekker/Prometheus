...
async def test_backtesting_validation():
    ...
    if report.overall_metrics:
        ...
        src_wr = report.win_rate * 100 if hasattr(report, 'win_rate') else 0
...
async def test_ai_capabilities():
    ...
    try:
        ...
        t0 = time.time()
        # Use the singleton engine's make_decision method if available
        ...
        has_answer = result is not None
        check("HRM reasoning completed", has_answer, f"{dt:.2f}s levels={[l.name for l in HRMReasoningLevel]}")
    ...
    try:
        ...
        # Create synthetic market data with exact column names MLRegimeDetector expects
        np.random.seed(42)
    ...
    try:
        ...
        t0 = time.time()
        # Use get_system_status as a quick capability check
    ...
    try:
        ...
        t0 = time.time()
        # Use available sync method to verify engine works
        if hasattr(optimizer, 'scan_arbitrage_opportunities'):
            ...
            result = {"engine": "QuantumTradingEngine", "status": "initialized",
                      "config": optimizer.config, "arbitrage_config": optimizer.arbitrage_config}
        dt = time.time() - t0
