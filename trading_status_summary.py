...
def check_system_status():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('enabled_globally', False):
                print("   [CHECK] Live trading is ENABLED globally")
            else:
                print("   [ERROR] Live trading is DISABLED globally")
                print("   🔧 Server needs to be restarted to pick up new config")
        else:
    ...
    if all_servers_running and ai_working > 0:
        print("   [CHECK] SYSTEM STATUS: OPERATIONAL")
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get('enabled_globally', False):
                    print("   [CHECK] LIVE TRADING: ENABLED")
                    print("   💰 Ready for real money trading")
                else:
                    print("   [WARNING]️ LIVE TRADING: DISABLED")
        ...
        print("   [ERROR] SYSTEM STATUS: NOT READY")
...
def provide_next_steps():
    ...
    print("- [CHECK] All servers running")
    print("- [CHECK] AI systems operational") 
    print("- [WARNING]️ Live trading needs server restart")
...
def main():
    ...
    if servers_ok and ai_ok:
        print("[CHECK] PROMETHEUS IS READY FOR TRADING")
        ...
        print("[ERROR] PROMETHEUS NEEDS ATTENTION")
