...
def add_openai_key():
    ...
    if not api_key:
        print("[WARNING]️  Skipping OpenAI key configuration")
        print("   You can add it later by editing the .env file")
        return False
    
    if not api_key.startswith('sk-'):
        print("[WARNING]️  Warning: OpenAI API keys usually start with 'sk-'")
        confirm = input("Continue anyway? (y/N): ").strip().lower()
        if confirm != 'y':
            print("[ERROR] Configuration cancelled")
            return False
    ...
    try:
        ...
        print("[CHECK] OpenAI API key added successfully!")
...
def main():
    ...
    if success:
        ...
        print()
        print("[WARNING]️  Configuration incomplete")
