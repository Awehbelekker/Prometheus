...
class AutonomousOvernightSystem:
    ...
    async def run_autonomous_enhancements(self):
        ...
        enhancements = [
            ...
            },
            {
                ...
                "function": self.continuous_monitoring,
                "priority": "LOW",
                "delay_minutes": 0,
                "description": "Monitor system health throughout night"
            }
        ]
        ...
        for enhancement in enhancements[:-1]:  # Skip monitoring as it's already started
            ...
            if self.verify_trading_active():
                ...
                try:
                    ...
                    if success:
                        ...
                        logger.warning(f"⚠️ Partial completion: {enhancement['name']}")
    ...
    def verify_trading_active(self):
        ...
        try:
            ...
            if 'python.exe' in result.stdout:
                # Find the trading process (original PID 46284)
                lines = result.stdout.split('\n')
                for line in lines:
                    if 'python.exe' in line:
                        pid = line.split()[1]
                        if self.trading_pid is None:
                            self.trading_pid = pid
                        elif pid == self.trading_pid:
                            return True
                return len([l for l in lines if 'python.exe' in l]) > 0
            return False
        except Exception as e:
    ...
    async def enhance_chart_analysis(self):
        ...
        try:
            ...
            if process.returncode == 0:
                logger.info("✅ Fresh chart analysis completed")
                ...
                logger.warning(f"⚠️ Chart analysis had issues but continuing")
                return False
    ...
    async def continuous_monitoring(self):
        ...
        while self.monitoring_active:
            try:
                ...
                if monitor_count % 20 == 0:  # Every 10 minutes (30sec * 20)
                    runtime = current_time - self.start_time
                    logger.info(f"📊 Monitor #{monitor_count//20}: Runtime {runtime}")
    ...
    async def generate_morning_summary(self):
        ...
        logger.info(f"🚀 Trading Status: {'ACTIVE' if self.verify_trading_active() else 'NEEDS ATTENTION'}")
